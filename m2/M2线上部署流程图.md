...
不要填写空的 `BOTTLENECK_RULES`：省略该键会使用代码内置的正式默认规则。`BATTERY_MAX_TEMPERATURE_FIELD` 和 `BATTERY_HOT_CLUSTER_FIELD` 暂时可留空；这只跳过温升判断，不会阻断三条链路低效率或逆变器低负载规则，也不会把缺失温度当成 0。环境变量名称见程序：`VIFA_EMU_URL`、`VIFA_EMU_TOKEN`、`M2_NOCOBASE_BASE_URL`、`M2_NOCOBASE_TOKEN`、`M2_TIMEZONE`、`M2_REQUEST_TIMEOUT_SECONDS`、`VIFA_GROWALL_URL`/`M2_GROWALL_URL`、`VIFA_GROWALL_TOKEN`/`M2_GROWALL_TOKEN`，以及各 `M2_*` 数值项。
...
只有在测试环境、数据表和权限已由现场人员确认后，才可进行人工写入。先不要在编辑器启用分钟 inject；在服务器终端只运行一个场站，例如：
