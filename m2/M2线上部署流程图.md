...
outbox 路径的本地配置键名是 `EVENT_OUTBOX_PATH`，环境变量键名是 `M2_EVENT_OUTBOX_PATH`；本说明不记录现场路径值。两者均未提供时，默认在 `energy-efficiency-api.py` 同目录创建 `energy_efficiency_event_outbox.sqlite3`。

不要填写空的 `BOTTLENECK_RULES`：省略该键会使用代码内置的正式默认规则。`PV_INVERTER_RATED_POWER_KW` 虽保留兼容配置入口，但只能是数值 `60`，其他值会作为安全配置错误拒绝。`BATTERY_MAX_TEMPERATURE_FIELD` 和 `BATTERY_HOT_CLUSTER_FIELD` 暂时可留空；这只跳过温升判断，不会阻断三条链路低效率或逆变器低负载规则，也不会把缺失温度当成 0。其他环境变量名称见程序：`VIFA_EMU_URL`、`VIFA_EMU_TOKEN`、`M2_NOCOBASE_BASE_URL`、`M2_NOCOBASE_TOKEN`、`M2_TIMEZONE`、`M2_REQUEST_TIMEOUT_SECONDS`、`VIFA_GROWALL_URL`/`M2_GROWALL_URL`、`VIFA_GROWALL_TOKEN`/`M2_GROWALL_TOKEN`，以及各 `M2_*` 数值项。
