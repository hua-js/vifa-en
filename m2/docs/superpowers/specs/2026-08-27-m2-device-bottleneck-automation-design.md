...
敏感配置只保存在服务器同目录的 `energy_efficiency_local_config.py`，不得写入 Git、日志或 JSON 响应。
...
```bash
python3 energy-efficiency-api.py minute ES01
python3 energy-efficiency-api.py minute ES02
```
...
```bash
python3 energy-efficiency-api.py cleanup
```
...
- `energy-efficiency-api.py`：CLI 参数、配置加载、统一 JSON 输出；
...
- `m2/场站三条能效链路能流图.html`：第三类事件和触发快照展示。
