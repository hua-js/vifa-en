...
**Goal:** Create a safe `energy-efficiency-api.py` command-line entrypoint that Node-RED can call to obtain one station's real-time dashboard JSON.
...
**Spec:** `m2/docs/superpowers/specs/2026-08-26-node-red-energy-efficiency-api-design.md`

## Global Constraints

- Create the deployable entrypoint at repository root as `energy-efficiency-api.py`.
...
**Files:**
- Create: `energy-efficiency-api.py`
...
ENTRYPOINT = Path(__file__).resolve().parents[2] / "energy-efficiency-api.py"
...
Expected: FAIL because `energy-efficiency-api.py` does not exist.

- [ ] **Step 3: Implement minimal parsing and configuration validation**

Create `energy-efficiency-api.py` with:
...
```bash
python3 -m unittest m2.tests.test_energy_efficiency_api
python3 energy-efficiency-api.py dashboard ES03
```
