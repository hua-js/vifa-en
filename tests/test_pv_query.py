...
from test_pv_publication import bundle
from test_pv_forecast_tools import load


def query():
    assert importlib.util.find_spec('m3_worker.services.pv_query') is not None,'query service missing'
    from m3_worker.services import pv_query
    return pv_query
...
class QueryAPITests(unittest.TestCase):
    def app(self,builder):
        assert importlib.util.find_spec('m3_worker.pv_query_app') is not None,'query app missing'
        from m3_worker.pv_query_app import create_app
