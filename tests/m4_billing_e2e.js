...
(async()=>{
 ...
 try{
  ...
  browser=await chromium.launch({headless:true});const page=await browser.newPage({viewport:{width:1440,height:1100},locale:'zh-CN'}),errors=[],writes=[],requests=[];let mode='normal';
  ...
  await page.route('**/bills?*',async route=>{
   ...
   if(mode==='hold'&&station==='station-1')await new Promise(r=>release=r);
  ...
  await page.setViewportSize({width:1440,height:1100});mode='partial';await page.locator('#billing-refresh').click();await page.waitForFunction(()=>document.querySelector('#billing-status').dataset.state==='partial');
  ...
  console.log('PASS: billing tab, monthly/day totals, historical month, negative earnings, CSV, partial/empty/error states, escaped source text, wrong-station rejection, late-response isolation, month memory, keyboard tabs, 8 light/dark layout screenshots; Mock GET only.');
