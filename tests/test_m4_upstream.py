...
from m4_settings.upstream import NocoBaseClient, SourceReadError
...
class UpstreamTests(unittest.TestCase):
    def read(self, *, opener=None, table='t_es_data', **kwargs):
        ...
        with patch('m4_settings.upstream.build_opener', return_value=opener):
    ...
    def test_empty_token_does_not_block_construction_but_blocks_read(self):
        for token in ('', ' ', None, 'unsafe\nsecret'):
            with self.subTest(token=token), patch('m4_settings.upstream.build_opener', return_value=FakeOpener()) as builder:
    ...
    def test_invalid_fields_fail_before_io(self):
        for fields in ('', '*', '../path', 'emu_sn&filter=secret', None):
            with self.subTest(fields=fields), patch('m4_settings.upstream.build_opener', return_value=FakeOpener()) as builder:
                with self.assertRaises(SourceReadError):
    ...
    def test_redirects_and_foreign_final_origin_are_rejected(self):
        with patch('m4_settings.upstream.build_opener', return_value=FakeOpener()) as builder:
    ...
    def test_whole_read_deadline_is_enforced(self):
        opener = FakeOpener()
        with patch('m4_settings.upstream.monotonic', side_effect=[0, 0, 31]):
            with self.assertRaises(SourceReadError):
