...
def manager_class():
    assert importlib.util.find_spec('m3_worker.services.pv_manual_jobs'), 'manual job service missing'
    from m3_worker.services.pv_manual_jobs import ManualJobs
    return ManualJobs
...
class ManualAPITests(unittest.TestCase):
    def test_auth_fixed_routes_and_empty_request_contract(self):
        assert importlib.util.find_spec('m3_worker.pv_service_app'), 'manual API missing'
        from m3_worker.pv_service_app import create_app
        with tempfile.TemporaryDirectory() as temp:
