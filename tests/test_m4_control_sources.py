...
from m4_settings.control_sources import ControlSourceError, ControlSourceReader
...
class ControlSourceTests(unittest.TestCase):
    def fetch(self, rows=None, station='station-1', **opener_options):
        ...
        with patch('m4_settings.control_sources.build_opener', return_value=opener):
    ...
    def test_control_policy_version_changes_source_version(self):
        first, _ = self.fetch()
        with patch('m4_settings.control_sources.CONTROL_POLICY_VERSION', 'different-confirmed-policy'):
            second, _ = self.fetch()
    ...
    def test_missing_token_invalid_station_and_non_https_origins_fail_before_io(self):
        with patch('m4_settings.control_sources.build_opener') as opener:
        ...
        with patch('m4_settings.control_sources.build_opener', return_value=FakeOpener(source_rows())) as opener:
    ...
    def test_redirect_handler_refuses_every_redirect(self):
        with patch('m4_settings.control_sources.build_opener', return_value=FakeOpener(source_rows())) as builder:
