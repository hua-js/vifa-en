...
from m4_orchestrator.contracts import M4OrchestrationResult
from m4_selection import select_candidate
from m4_selection.__main__ import _reject_constant, _unique_pairs
from m4_settings.models import StationConfiguration
from m4_settings.selection import LiveSelectionResult, StationPolicy
...
def verify_request_inputs(envelope, configuration, request):
    ...
    from m4_settings.selection import _decision_content
...
def main(argv=None):
    ...
    run_id = str(uuid4())
    root = Path(os.environ.get('M4_DECISION_RESULTS_DIR', str(ROOT / 'm4/run/solver-decisions')))
