...
**Spec:** `docs/superpowers/specs/2026-08-31-m3-weekly-load-forecast-design.md`
...
**Files:**
- Modify: `m3_worker/domain/custom_training_data.py:17-132`
- Create: `tests/test_m3_custom_training_data.py`
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_custom_training_data
```
...
.venv/bin/python -m unittest -v \
  tests.test_m3_custom_training_data \
  tests.test_m3_training_data
```
...
```bash
git add m3_worker/domain/custom_training_data.py tests/test_m3_custom_training_data.py
...
**Files:**
- Create: `m3_worker/domain/custom_load_profiles.py`
- Create: `tests/test_m3_custom_load_profiles.py`
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_custom_load_profiles
```
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_custom_load_profiles
git add m3_worker/domain/custom_load_profiles.py tests/test_m3_custom_load_profiles.py
...
**Files:**
- Modify: `m3_worker/domain/custom_forecasting.py:22-249`
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_custom_forecasting
```
...
.venv/bin/python -m unittest -v \
  tests.test_m3_custom_training_data \
  tests.test_m3_custom_load_profiles \
  tests.test_m3_custom_forecasting \
  tests.test_m3_forecasting
git add m3_worker/domain/custom_forecasting.py tests/test_m3_custom_forecasting.py
...
**Files:**
- Modify: `m3_worker/services/custom_forecast_service.py:177-278`
- Modify: `m3_worker/services/custom_forecast_repository.py:423-475`
- Create: `tests/test_m3_custom_forecast_service.py`
- Modify: `tests/test_m3_custom_forecast_repository.py`
...
**Files:**
- Modify: `m3_worker/services/custom_forecast_evaluation_service.py:290-365`
- Create: `tests/test_m3_custom_forecast_evaluation.py`
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_custom_forecast_evaluation
```
...
```bash
.venv/bin/python -m unittest -v \
  tests.test_m3_custom_forecast_evaluation \
  tests.test_m3_custom_forecast_repository
```
...
```bash
git add \
  m3_worker/services/custom_forecast_evaluation_service.py \
  tests/test_m3_custom_forecast_evaluation.py
...
- Modify: `tests/test_m3_deployment_artifacts.py:24-36,400-416`
- Modify: `tests/m3_dashboard_e2e.js:1-20`

**Interfaces:**
- Treats: `m3/node_red/m3_production_gateway_page.html` as the canonical complete importable HTML.
- Leaves: `front/场站未来能耗预测.html` unchanged and no longer treats it as the production source.
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_deployment_artifacts
node --test tests/m3_dashboard_e2e.js
```
...
```bash
.venv/bin/python -m unittest -v tests.test_m3_deployment_artifacts
node --test tests/m3_dashboard_e2e.js tests/test_m3_node_red_contract.js
git add \
  ...
  tests/test_m3_deployment_artifacts.py \
  tests/m3_dashboard_e2e.js
...
```bash
.venv/bin/python -m compileall -q m3_worker tests
git diff --check
git status --short
```
