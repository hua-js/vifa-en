...
新增服务器入口文件 `m2/energy-efficiency-api.py`，由 Node-RED 的 `exec` 节点按请求启动一次。入口文件读取一个场站的实时数据，调用现有 M2 计算模块，并向标准输出返回 HTML 看板能够直接使用的单行 JSON。
...
```text
m2/node_red/node-red-energy-efficiency-api-flow.json
```
