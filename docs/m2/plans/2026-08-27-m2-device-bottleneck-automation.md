...
**Spec:** `docs/m2/specs/2026-08-27-m2-device-bottleneck-automation-design.md`
...
- Secrets remain only in `.local/energy_efficiency_local_config.py` or environment variables and must never appear in Git, logs, exception text, URLs, or JSON responses.
...
```bash
python3 -m unittest m2.tests.test_energy_efficiency_api -v
python3 m2/energy-efficiency-api.py invalid 2>&1 | python3 -c "import json,sys; lines=sys.stdin.read().splitlines(); assert len(lines)==1; assert json.loads(lines[0])['status']=='error'"
```
...
**Files:**
- Modify: `m2/web/场站三条能效链路能流图.html`
...
```bash
git add 'm2/web/场站三条能效链路能流图.html' m2/tests/energy_dashboard_e2e.js
...
**Files:**
- Modify: `m2/node_red/node-red-energy-efficiency-api-flow.json`
- Modify: `docs/m2/M2线上部署流程图.md`
- Modify: `docs/m2/M2未完成事项.md`
...
```bash
python3 -m json.tool m2/node_red/node-red-energy-efficiency-api-flow.json >/dev/null
```
...
```text
iframe → Node-RED /energy-efficiency-api → exec m2/energy-efficiency-api.py dashboard <station>
...
```bash
git add m2/node_red/node-red-energy-efficiency-api-flow.json \
  'docs/m2/M2线上部署流程图.md' \
  'docs/m2/M2未完成事项.md'
...
git status --short
git diff --stat
rg -n "Bearer eyJ|PASSWORD_PLACEHOLDER|Authorization: Bearer" --glob '!.local/energy_efficiency_local_config.py' .
```
