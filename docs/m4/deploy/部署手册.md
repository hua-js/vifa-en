...
Docker 使用已有登录或凭据助手；脚本不读取或保存登录密码。可通过 `CRR_IMAGE` 更换仓库，发布标签固定为 `0.1.0`，`RELEASE_DIR` 指定尚不存在的输出目录，`PYTHON_IMAGE` 指定构建基础镜像。平台固定 `linux/amd64`，适配本地 Mac 构建、生产 AMD64 运行。

脚本导出指定 Git 提交的快照，按白名单生成部署包，在本地 `buildx build --load`，核对镜像架构后推送两种标签并检查远端 manifest：

- 发布标签：`vifa-m4:0.1.0`。
- 提交标签：`vifa-m4:m4-<12位Git提交号>-amd64`。

默认输出到仓库旁 `outputs/m4/releases/0.1.0-<12位提交号>-<时间>-<进程号>/` 及同名 ZIP。包内 `release.json` 记录完整提交号和镜像，`backend/.env.example` 自动填入固定标签。生产每次拉取固定标签并重建容器，提交标签保留用于回滚。构建或推送失败时脚本退出非零，不打印发布成功；先排查错误，直接重试（默认自动生成新目录）。生成了 ZIP 不代表镜像已经推送成功。
...
docker run --rm --platform linux/amd64 --network none --read-only \
  ...
  -v vifa-m4-data:/data:ro ccr.ccs.tencentyun.com/taidai-holobase-168/vifa-m4:0.1.0 \
...
- 更新后端：本地提交后直接运行 `bash m4/deploy/build-and-push.sh`，固定发布 `vifa-m4:0.1.0`（远端加腾讯CCR仓库前缀）。服务器确认无运行中任务并备份，将新包内容合并到原安装目录，保留已有 `.env`、Token 和数据卷；仅将 `backend/.env` 中的 `M4_IMAGE` 改为新包 `release.json` 对应的固定标签，再执行以下命令。
...
docker save -o vifa-m4-amd64.tar ccr.ccs.tencentyun.com/taidai-holobase-168/vifa-m4:0.1.0
# 服务器：
docker load -i vifa-m4-amd64.tar
# .env 的 M4_IMAGE 设置为已导入的同一完整标签后：
docker compose up -d --no-build --force-recreate m4-api
```
