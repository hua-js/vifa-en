...
往日历史处理：用户已确认 EMS 支持 `repeat=已过期`。跨日检查将往日带 `m4_run_id` 和有效 `m4_plan_date` 的记录更新为“已过期”，保留原日期、时段、功率和计划 ID，不删除；已过期记录不参与当天读取及替换匹配。当天冲突记录的替换／删除保持原规则，不增加配置开关。已补充回归用例，未运行测试、未部署或生产写入；本轮未独立验证 EMS 执行。
...
生产更新直接使用发布脚本打印的完整命令，避免此处维护第二套命令。电站2联调保留三份Compose覆盖。脚本会备份并补齐旧覆盖文件中缺失的自动计划开关；已有显式关闭配置不会被覆盖，预检查会停止部署。拉取后先核对镜像内容摘要，并用一次性Python进程检查自动计划、EMS写表开关和电站2实测充电/SOC准入及充电下发配置，再重建服务。重建后核对运行镜像、配置及健康状态；旧镜像加本地回退标签保留，不自动清理。外部project.json与镜像内置的上述配置不一致时须先人工同步。
