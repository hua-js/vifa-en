...
线上502通过容器检查定位：M4仅在vifa-m4_default，Node-RED在1panel-network，没有共同网络。最新m4/deploy/m4-station-power.override.yaml显式保留default并加入外部1panel-network，别名m4-api。将此文件放服务器backend目录并最后应用，保留原PV/M3 MAPE覆盖文件和数据挂载；不能只覆盖image而依赖线上旧PV文件含网络配置。Node-RED后端地址使用http://m4-api:8844。

现有容器可用docker network connect --alias m4-api 1panel-network vifa-m4-m4-api-1临时恢复连接，重建持久性依赖上述Compose配置。需从nodered容器访问settings返回200并回查公开API后确认恢复。
