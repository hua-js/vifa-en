...
默认输出到仓库旁 `outputs/m4/releases/m4-0.1.0-<12位提交号>/` 及同名 ZIP。包内 `release.json` 记录完整提交号和镜像，`backend/.env.example` 自动填入提交标签。生产建议使用该提交标签，便于固定版本和回滚。构建或推送失败时脚本退出非零，不打印发布成功；先排查错误，再使用新的 `RELEASE_DIR` 重试。生成了 ZIP 不代表镜像已经推送成功。
...
本次发布沿用现有CCR镜像与Compose部署；以下命令仅由用户在服务器执行。标签为`m4-cabinet-allocation-20260910-amd64`，已回读摘要`sha256:4cf6cc06eecc80fa7fef80a56403dff5832f943c58397b9f1a8d0b02e083c2a5`，平台linux/amd64。源码清单在`outputs/m4/releases/cabinet-allocation-20260910/source-manifest.json`。未远程部署、未接设备下发。

将`m4/deploy/m4-cabinet-allocation.override.yaml`上传到`/userdata/holo/pyfiles/m4/backend/`，然后执行：

```bash
cd /userdata/holo/pyfiles/m4/backend
m4compose() {
  docker compose -f compose.yaml \
    -f m4-pv.override.yaml \
    -f m4-m3-mape.override.yaml \
    -f m4-pv-readonly.override.yaml \
    -f m4-cabinet-allocation.override.yaml "$@"
}
m4compose config --quiet &&
m4compose pull m4-api &&
m4compose up -d --no-deps --no-build m4-api
m4compose ps m4-api
m4compose logs --tail=50 m4-api
```

此覆盖层替代此前仅设置镜像/网络的late-peak覆盖层，保留原PV、MAPE及只读数据挂载。`1panel-network`中保持`m4-api`别名，避免重建后Node-RED出现502。柜级档案存于现有持久化`/data/cabinet-allocations`，沿用/data的主机绑定和10001写权限；不增加匿名卷、不清理原solver证据。

Node-RED需要同步两个现有节点：

1. 将`m4/node_red/m4_prepare_proxy.js`完整替换现有“限定接口与内部转发”函数内容，放行新增GET/POST分配路由；保留其他认证节点、环境变量及连接。
2. 将`m4/web/M4优化调度控制台-线上版.html`完整替换现有页面Template节点内容，部署Node-RED。无需导入整份Flow，避免覆盖现场模板或连接。

打开线上M4，在已有当天有效站级计划上点击“生成并保存分配”。保存后刷新应读取同一快照，点击“分配记录”查看时间与逐柜结果。计划已过期或参数已变更则先生成新站级计划。刷新或查看历史不会生成新分配；当前仍无自动分配、设备下发和执行回读。

本地8847仍是HTML+线上GET的只读预览，不代理POST；保存操作应在线上认证页面进行。

验证范围：29项相关后端测试在本机与断网只读amd64镜像均通过，19项Node-RED测试通过；59个镜像源码哈希一致。隔离页面保存/刷新/历史及明暗模式、桌面/平板/手机布局已检查。完整M4回归仍有37项既有失败，以修改前API复跑逐项重现；本次没有修复这些历史测试。日志在`outputs/m4/evaluations/2026-09-10-allocation-validation/`。
