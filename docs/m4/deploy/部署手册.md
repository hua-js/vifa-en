...
cp .env.example .env
# 按实际情况编辑 .env 的 M4_IMAGE、端口与数据卷名；M4_IMAGE 必填，否则 compose 会报错退出。
...
cd /userdata/holo/pyfiles/m4/backend
M4_COMPOSE_FILES=(-f compose.yaml -f m4-production.override.yaml)
docker compose "${M4_COMPOSE_FILES[@]}" config --images   # 确认是本次发布标签
docker compose "${M4_COMPOSE_FILES[@]}" pull m4-api
docker compose "${M4_COMPOSE_FILES[@]}" up -d --no-build m4-api
...
```bash
cd /userdata/holo/pyfiles/m4/backend
docker compose -f compose.yaml -f m4-production.override.yaml up -d --no-build m4-api
```
