...
> 产品化改造后的配置与发布方式以 [产品化配置与交付](../../产品化配置与交付.md) 为准：ARM64/AMD64通过显式平台参数选择，部署包绑定提交镜像，项目JSON独立挂载，Compose首次启动默认关闭自动任务。下文固定AMD64、固定两站、固定数据源和固定0.1.0标签的内容是旧项目操作参考，不能直接作为新项目发布步骤。新版必须同步发布`/m4-api/project`网关、HTML和后端。
