...
M4 阶段 A 已完成独立的 `m4.optimizer` 数学优化核心：单个电站输入一份完整的 96 点请求，输出 `balanced`、`cost`、`pv` 三套候选，并对计划和指标执行确定性复验。
...
- 在仓库内新增独立的 `m4.orchestrator` Python 包，但不拆分独立微服务；
...
`m4.orchestrator` 是仓库内可导入模块，不是独立部署单元。CLI 是第一个调用方；未来 FastAPI 只需调用同一个服务入口并序列化同一输出模型。
...
├── __main__.py       # 支持 python -m m4.orchestrator
...
每个 `--input` 文件包含一份完整的 `m4.optimizer.OptimizationRequest` JSON。字段名、严格类型、96 点连续时间轴、来源版本、能力快照、约束和三套目标配置均复用阶段 A 契约，不建立第二套近似输入模型。
...
- `m4.orchestrator` 可作为 Python 包导入，并可通过 `python -m m4.orchestrator` 运行；
