...
**Spec:** `docs/m4/specs/2026-09-04-m4-mathematical-optimizer-design.md`
...
- Modify: `uv.lock`
- Create: `m4/optimizer/__init__.py`
- Create: `m4/optimizer/contracts.py`
- Create: `m4/tests/m4_optimizer_test_support.py`
- Create: `m4/tests/test_m4_optimizer_contracts.py`
...
在 `m4/tests/test_m4_optimizer_contracts.py` 中覆盖以下行为：
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_contracts.py -v
```

Expected: FAIL，原因是 `m4.optimizer.contracts` 或测试工厂尚不存在。
...
在 `m4/optimizer/contracts.py` 中定义以下稳定字段和字面量；所有模型使用 `ConfigDict(extra="forbid", strict=True)`，所有数值字段禁止 `NaN` 和无穷值：
...
在 `m4/tests/m4_optimizer_test_support.py` 中用固定 `+08:00` 时间、平坦负荷/光伏和三份完整目标配置构建 `make_request(**overrides)`。工厂必须创建新的列表和模型，不能在测试之间共享可变对象。测试目标配置由以下函数创建：
...
在 `m4/optimizer/__init__.py` 中只导出契约类型；求解入口留到 Task 6 再加入。
...
```bash
git add pyproject.toml uv.lock m4/optimizer/__init__.py m4/optimizer/contracts.py m4/tests/m4_optimizer_test_support.py m4/tests/test_m4_optimizer_contracts.py
...
**Files:**
- Create: `m4/optimizer/solver.py`
- Create: `m4/tests/test_m4_optimizer_solver.py`

**Interfaces:**
- Consumes: `CandidateStatus` from `m4.optimizer.contracts`。
...
from m4.optimizer.solver import MilpProblem, solve_milp
...
class M4OptimizerSolverTests(unittest.TestCase):
    ...
    def test_time_limited_finite_incumbent_is_feasible(self):
        ...
        with patch("m4.optimizer.solver.milp", return_value=fake):
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_solver.py -v
```

Expected: FAIL，原因是 `m4.optimizer.solver` 尚不存在。

- [ ] **Step 3: 实现求解器适配层**

在 `m4/optimizer/solver.py` 中使用以下数据结构：
...
```bash
git add m4/optimizer/solver.py m4/tests/test_m4_optimizer_solver.py
...
**Files:**
- Create: `m4/optimizer/model.py`
- Create: `m4/tests/test_m4_optimizer_model.py`
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_model.py -v
```

Expected: FAIL，原因是 `m4.optimizer.model` 尚不存在。

- [ ] **Step 3: 实现变量索引和稀疏行构建器**

在 `m4/optimizer/model.py` 定义：
...
from m4.optimizer.solver import MilpProblem
...
```bash
git add m4/optimizer/model.py m4/tests/test_m4_optimizer_model.py
...
**Files:**
- Create: `m4/optimizer/lexicographic.py`
- Create: `m4/tests/test_m4_optimizer_lexicographic.py`
...
from m4.optimizer.contracts import ObjectiveLayer, ObjectiveProfile
from m4.optimizer.lexicographic import solve_profile
from m4.optimizer.model import BuiltModel, VariableIndex
from m4.optimizer.solver import MilpProblem
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_lexicographic.py -v
```

Expected: FAIL，原因是 `m4.optimizer.lexicographic` 尚不存在。

- [ ] **Step 3: 实现分层求解**

在 `m4/optimizer/lexicographic.py` 定义：
...
from m4.optimizer.contracts import CandidateStatus, LayerResult, ObjectiveLayer, ObjectiveProfile
from m4.optimizer.model import BuiltModel
from m4.optimizer.solver import ObjectiveLock, solve_milp
...
```bash
git add m4/optimizer/lexicographic.py m4/tests/test_m4_optimizer_lexicographic.py
...
**Files:**
- Create: `m4/optimizer/metrics.py`
- Create: `m4/optimizer/validation.py`
- Create: `m4/tests/test_m4_optimizer_validation.py`
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_validation.py -v
```
...
```bash
git add m4/optimizer/metrics.py m4/optimizer/validation.py m4/tests/m4_optimizer_test_support.py m4/tests/test_m4_optimizer_validation.py
...
**Files:**
- Create: `m4/optimizer/profiles.py`
- Create: `m4/optimizer/service.py`
- Modify: `m4/optimizer/__init__.py`
- Create: `m4/tests/test_m4_optimizer_scenarios.py`
...
```bash
uv run python -m unittest m4/tests/test_m4_optimizer_scenarios.py -v
```
...
在 `m4/optimizer/__init__.py` 导出 `M4Optimizer` 和稳定契约。
...
```bash
git add m4/optimizer/profiles.py m4/optimizer/service.py m4/optimizer/__init__.py m4/tests/test_m4_optimizer_scenarios.py
...
**Files:**
- Modify: `m4/tests/m4_optimizer_test_support.py`
- Modify: `m4/tests/test_m4_optimizer_scenarios.py`
- Create: `docs/m4/optimizer/README.md`
- Create: `docs/m4/M4数学优化器阶段A验收说明.md`
...
在 `m4/tests/test_m4_optimizer_scenarios.py` 增加以下独立场景：
...
在 `m4/tests/m4_optimizer_test_support.py` 增加确定性工厂：
...
`docs/m4/optimizer/README.md` 必须包含：
...
`docs/m4/M4数学优化器阶段A验收说明.md` 必须把本任务中的业务场景逐项映射到测试方法名，并声明阶段 A 只证明数学候选生成和确定性复算，不证明真实节费比例、AI 选择效果或 EMS 设备执行效果。
...
```bash
/usr/bin/time -p uv run python -m unittest m4/tests/test_m4_optimizer_scenarios.py -v
```
...
```bash
git add m4.optimizer m4/tests/m4_optimizer_test_support.py m4/tests/test_m4_optimizer_contracts.py m4/tests/test_m4_optimizer_solver.py m4/tests/test_m4_optimizer_model.py m4/tests/test_m4_optimizer_lexicographic.py m4/tests/test_m4_optimizer_validation.py m4/tests/test_m4_optimizer_scenarios.py docs/m4/M4数学优化器阶段A验收说明.md pyproject.toml uv.lock
...
- `m4.optimizer` 可以在不启动 FastAPI、不连接数据库、不调用 AI/EMS 的情况下生成三套候选；
