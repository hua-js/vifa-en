...
**Goal:** Build a repository-local `m4.orchestrator` package that reads independent station JSON inputs, invokes the real M4 optimizer per station, and atomically writes one strict offline result containing all candidate plans while leaving AI and EMS inactive.
...
**Tech Stack:** Python 3.12, Pydantic 2.13.4, existing `m4.optimizer`, `argparse`, `json`, `tempfile`, `unittest`.

**Spec:** `docs/m4/specs/2026-09-04-m4-offline-orchestration-design.md`
...
- Reuse `m4.optimizer.OptimizationRequest`, `M4Optimizer`, and `OptimizationResult`; do not duplicate or weaken their validation.
...
- `m4/orchestrator/__init__.py`: stable public exports.
- `m4/orchestrator/__main__.py`: `python -m m4.orchestrator` entry point.
- `m4/orchestrator/contracts.py`: strict orchestration input, status, error, summary, station, and top-level result models.
- `m4/orchestrator/loader.py`: isolated UTF-8/JSON/Pydantic loading and safe hint extraction.
- `m4/orchestrator/service.py`: duplicate rejection, per-station optimization, availability classification, ordering, and overall status.
- `m4/orchestrator/writer.py`: deterministic JSON bytes and same-directory atomic replacement.
- `m4/orchestrator/cli.py`: argument parsing, dependency composition, output, and exit codes.
- `docs/m4/orchestrator/README.md`: local usage, contract, failure semantics, and extension boundary.
...
- `docs/m4/M4离线编排层阶段B1验收说明.md`: acceptance evidence and non-production boundary.

### Tests

- `m4/tests/m4_orchestrator_test_support.py`: builders for loaded inputs, optimizer results, clocks, IDs, and fakes.
- `m4/tests/test_m4_orchestrator_contracts.py`: strict model invariants.
- `m4/tests/test_m4_orchestrator_loader.py`: file and validation error classification.
- `m4/tests/test_m4_orchestrator_service.py`: station isolation, ordering, statuses, determinism, and duplicate rejection.
- `m4/tests/test_m4_orchestrator_cli.py`: writer atomicity, CLI output, exit codes, and committed artifact acceptance.
...
**Files:**
- Create: `m4/orchestrator/__init__.py`
- Create: `m4/orchestrator/contracts.py`
- Create: `m4/tests/m4_orchestrator_test_support.py`
- Create: `m4/tests/test_m4_orchestrator_contracts.py`

**Interfaces:**
- Consumes: `m4.optimizer.OptimizationRequest` and `m4.optimizer.OptimizationResult`.
...
The test support module must provide fixed `UTC` clocks and a minimal valid `OptimizationResult` with an empty candidate list for structural contract tests. It must import `make_request()` from `m4/tests/m4_optimizer_test_support.py` instead of copying the optimizer request builder.
...
```bash
.venv/bin/python -m unittest m4/tests/test_m4_orchestrator_contracts.py -v
```

Expected: import failure for missing `m4.orchestrator`.
...
`m4/orchestrator/__init__.py` must export all public contract types. Do not export loader or CLI internals in Task 1.
...
.venv/bin/python -m unittest \
  m4/tests/test_m4_orchestrator_contracts.py \
  m4/tests/test_m4_optimizer_contracts.py -v
```
...
```bash
git add m4/orchestrator/__init__.py m4/orchestrator/contracts.py m4/tests/m4_orchestrator_test_support.py m4/tests/test_m4_orchestrator_contracts.py
...
**Files:**
- Create: `m4/orchestrator/loader.py`
- Create: `m4/tests/test_m4_orchestrator_loader.py`
- Modify: `m4/orchestrator/__init__.py`
...
```bash
.venv/bin/python -m unittest m4/tests/test_m4_orchestrator_loader.py -v
```
...
```bash
.venv/bin/python -m unittest \
  m4/tests/test_m4_orchestrator_loader.py \
  m4/tests/test_m4_orchestrator_contracts.py -v
```
...
```bash
git add m4/orchestrator/__init__.py m4/orchestrator/loader.py m4/tests/test_m4_orchestrator_loader.py
...
**Files:**
- Create: `m4/orchestrator/service.py`
- Create: `m4/tests/test_m4_orchestrator_service.py`
- Modify: `m4/orchestrator/__init__.py`
- Modify: `m4/tests/m4_orchestrator_test_support.py`
...
```bash
.venv/bin/python -m unittest m4/tests/test_m4_orchestrator_service.py -v
```
...
```bash
.venv/bin/python -m unittest \
  m4/tests/test_m4_orchestrator_service.py \
  m4/tests/test_m4_optimizer_scenarios.py -v
```
...
```bash
git add m4/orchestrator/__init__.py m4/orchestrator/service.py m4/tests/m4_orchestrator_test_support.py m4/tests/test_m4_orchestrator_service.py
...
**Files:**
- Create: `m4/orchestrator/writer.py`
- Create: `m4/orchestrator/cli.py`
- Create: `m4/orchestrator/__main__.py`
- Create: `m4/tests/test_m4_orchestrator_cli.py`
- Modify: `m4/orchestrator/__init__.py`
...
def test_replace_failure_preserves_existing_target_and_removes_temp_file(self):
    ...
    with patch("m4.orchestrator.writer.os.replace", side_effect=OSError("blocked")):
...
- `python -m m4.orchestrator --help` is available through `__main__.py` without imports that perform I/O.

- [ ] **Step 2: Run CLI tests and verify RED**

```bash
.venv/bin/python -m unittest m4/tests/test_m4_orchestrator_cli.py -v
```
...
.venv/bin/python -m unittest \
  m4/tests/test_m4_orchestrator_loader.py \
  m4/tests/test_m4_orchestrator_service.py \
  m4/tests/test_m4_orchestrator_cli.py -v
```
...
```bash
git add m4/orchestrator/__init__.py m4/orchestrator/__main__.py m4/orchestrator/writer.py m4/orchestrator/cli.py m4/tests/test_m4_orchestrator_cli.py
...
- Modify: `m4/tests/test_m4_orchestrator_service.py`
- Modify: `m4/tests/test_m4_orchestrator_cli.py`
...
```bash
.venv/bin/python -m unittest \
  m4.tests.test_m4_orchestrator_service.M4OrchestratorRealAcceptanceTests \
  m4.tests.test_m4_orchestrator_cli.M4OrchestratorArtifactTests -v
```
...
```bash
git add m4/mock/orchestration m4/tests/test_m4_orchestrator_service.py m4/tests/test_m4_orchestrator_cli.py
...
**Files:**
- Create: `docs/m4/orchestrator/README.md`
- Create: `docs/m4/M4离线编排层阶段B1验收说明.md`
- Modify only if verification exposes a real defect: the smallest relevant `m4/orchestrator/*.py`, test, or Mock JSON file.
...
`docs/m4/M4离线编排层阶段B1验收说明.md` must map each accepted requirement to a test or artifact, list both station fixture differences, link the input/output examples, record actual verification commands and counts, and state:
...
/usr/bin/time -p .venv/bin/python -B -m unittest \
  m4.tests.test_m4_orchestrator_service.M4OrchestratorRealAcceptanceTests -v
```
...
```bash
.venv/bin/python -B -m compileall -q m4.orchestrator
...
```bash
git add docs/m4/orchestrator/README.md docs/m4/M4离线编排层阶段B1验收说明.md
...
- `m4.orchestrator` imports without file, network, database, AI, or EMS side effects.
...
- Existing `m4.optimizer` behavior remains unchanged.
