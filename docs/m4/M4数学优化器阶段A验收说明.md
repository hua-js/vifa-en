...
阶段 A 不证明真实节费比例、最终候选选择效果或 EMS 设备执行效果；本阶段没有连接 AI、EMS、数据库或真实设备，也没有修改 M4 静态页面。
...
- 聚焦最终审查回归：`.venv/bin/python -m unittest m4/tests/test_m4_optimizer_contracts.py m4/tests/test_m4_optimizer_solver.py m4/tests/test_m4_optimizer_lexicographic.py m4/tests/test_m4_optimizer_model.py m4/tests/test_m4_optimizer_validation.py m4/tests/test_m4_optimizer_scenarios.py -v`，`Ran 66 tests in 7.299s`，0 失败，0 跳过。
...
```bash
/usr/bin/time -p .venv/bin/python -m unittest m4/tests/test_m4_optimizer_scenarios.py -v
```
...
- 数学候选仍需按经营偏好进行数学选择、EMS 安全校核和设备闭环验证后，才能进入真实控制链路。
