...
test('retired allocation routes cannot reach the backend', () => {
    for (const resource of ['allocations', 'allocation-result']) {
        for (const method of ['GET', 'POST']) {
            rejected('prepare_proxy', request({method, resource, query: {allocation_id:runId, run_id:runId}, payload:{request_id:runId,plan_run_id:runId}}), 404);
        }
    }
});
