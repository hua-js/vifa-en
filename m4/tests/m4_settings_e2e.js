...
(async()=>{
 ...
 const server=spawn(path.join(root,'.venv/bin/python'),['-m','uvicorn','m4.settings.api:create_app','--factory','--host','127.0.0.1','--port','0'],{cwd:root,env:{...process.env,M4_SETTINGS_DB:path.join(temp,'settings.sqlite3'),M4_NOCOBASE_TOKEN:''},stdio:['ignore','pipe','pipe']});
 let browser;
 try {
