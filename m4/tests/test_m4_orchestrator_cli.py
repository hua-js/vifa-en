...
from m4.orchestrator import M4OrchestrationResult, M4Orchestrator, StationInput
from m4.orchestrator.cli import main
from m4.orchestrator.writer import (
    OutputWriteError,
    serialize_result,
    write_result_atomic,
)
...
class M4OrchestratorWriterTests(unittest.TestCase):
    ...
    def test_replace_failure_preserves_existing_target_and_removes_temp_file(self):
        ...
        target.write_text("old-result\n", encoding="utf-8")
        with patch(
            "m4.orchestrator.writer.os.replace",
    ...
    def test_interrupts_propagate_after_only_new_temp_is_removed(self):
        ...
        for index, interruption in enumerate(interruptions, start=1):
            with self.subTest(interruption=type(interruption).__name__):
                ...
                with patch(
                    "m4.orchestrator.writer.os.fsync",
    ...
    def test_path_recording_interrupt_closes_and_removes_only_owned_temp(self):
        ...
        for index, interruption in enumerate(interruptions, start=1):
            with self.subTest(interruption=type(interruption).__name__):
                ...
                try:
                    with patch(
                        "m4.orchestrator.writer.NamedTemporaryFile",
                        side_effect=recording_named_temporary_file,
                    ), patch(
                        "m4.orchestrator.writer.Path",
    ...
    def test_ordinary_failure_raises_fresh_redacted_error_without_chain(self):
        ...
        with patch(
            "m4.orchestrator.writer.serialize_result",
...
class M4OrchestratorCliTests(unittest.TestCase):
    ...
    def invoke(
        ...
        *,
        ...
        stderr = StringIO()
        with patch(
            "m4.orchestrator.service.M4Optimizer",
    ...
    def test_blank_explicit_versions_exit_two_before_io_service_or_write(self):
        ...
        for argument, invalid_value, argv in cases:
            with self.subTest(argument=argument, value=invalid_value):
                ...
                stderr = StringIO()
                with patch(
                    "m4.orchestrator.cli.load_station_input"
                ) as load_input, patch(
                    "m4.orchestrator.cli.M4Orchestrator"
                ) as orchestrator, patch(
                    "m4.orchestrator.cli.write_result_atomic"
                ) as write_result:
                    with redirect_stdout(stdout), redirect_stderr(stderr):
                        with self.assertRaises(SystemExit) as raised:
                            main(argv)
    ...
    def test_module_help_is_available_without_running_orchestration(self):
        completed = subprocess.run(
            [sys.executable, "-m", "m4.orchestrator", "--help"],
