...
const generated=spawnSync(path.join(root,'.venv/bin/python'),['-m','m4.tests.m4_peak_preparation_fixtures'],{cwd:root,encoding:'utf8'});assert.equal(generated.status,0,generated.stderr);const peaks=JSON.parse(generated.stdout);
...
(async()=>{
 const host=spawn(path.join(root,'.venv/bin/python'),['m4/tests/m4_selection_demo_server.py'],{cwd:root});
