...
from m4.tests.test_m4_realtime import cabinet, configuration
...
class LiveInputTests(unittest.TestCase):
    ...
    def test_missing_tariff_labels_cannot_be_reported_as_ready_source(self):
        with patch('m4.settings.live_inputs.LiveInputService._tariff', return_value={
    ...
    def test_actual_clock_crossing_plan_boundary_blocks_bundle(self):
        with patch('m4.settings.live_inputs._clock_now', side_effect=[NOW, START+timedelta(seconds=1)]):
    ...
    def test_request_rechecks_wall_clock_and_observation_when_reusing_bundle(self):
        ...
        with patch('m4.settings.live_inputs._clock_now', return_value=START+timedelta(days=1)):
            with self.assertRaises(ValueError):
