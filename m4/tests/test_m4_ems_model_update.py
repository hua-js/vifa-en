...
from m4.settings.terminal_policy import POLICY as TERMINAL_POLICY, ROLLING_REFERENCE_POLICY
...
class ModelUpdateTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.payload = dict(schema_version='m4-rolling-plan-v1', station_id='station-2',
            ...
            request=dict(station_id='station-2',
                peak_reserve_policy=dict(version='peak-reserve-v4', terminal_soc_min_pct=5), capability=dict(available=True, max_charge_kw=600, max_discharge_kw=540,
                ...
                source_versions=dict(configuration='v1', project_configuration=get_project().fingerprint,
                    grid_charging_policy=GRID_CHARGING_POLICY, terminal_policy=TERMINAL_POLICY,
                    rolling_reference_policy=ROLLING_REFERENCE_POLICY)),
    ...
    def test_preview_binds_only_id7_and_station_and_version(self):
        ...
        self.assertEqual(request['body'], dict(start_time='14:15:00', end_time='14:30:00',
            type='discharge', kw=90, repeat='今日有效', es_sn=['ES02']))
        self.assertFalse(result['network_write_performed'])
        self.opener.open.assert_not_called()

    def test_stale_terminal_versions_are_blocked_before_table_read(self):
        for key in ('terminal_policy', 'rolling_reference_policy'):
            with self.subTest(key=key):
                current = self.payload['request']['source_versions'].pop(key)
                try:
                    with self.assertRaisesRegex(ModelUpdateError, '日末电量规则'):
                        self.preview()
                    self.reader._read_table.assert_not_called()
                finally:
                    self.payload['request']['source_versions'][key] = current
