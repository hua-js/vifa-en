...
版本：2026-09-09 TCP / CCR 发布版。部署方式：**Node-RED 提供 HTML 并校验 iframe Token，NocoBase 内嵌 URL，Linux AMD64 服务器运行 Docker 后端**。复用现有 Node-RED 的 HTTPS 访问地址，不新增 Nginx。

当前页面包含运行总览、调度计划、设备与数据、运行记录、账单统计。M4 生成调度预览，尚未接入设备下发、执行回读或自动滚动调度。

本版恢复 TCP 通信，取消 Socket 目录挂载及 m4Http 配置要求。Node-RED 使用内置 HTTP Request 节点；iframe 专用 Token 和数据卷仍按下文配置。
...
NocoBase iframe → https://你的Node-RED域名/m4?token=访问Token&station=s1
                    ...
                    └─ 页面 /m4-api 请求携带 Bearer
                         → Node-RED 校验 Token → Docker 后端 :8844
                                                    ├─ 持久化参数与历史
                                                    └─ 只读访问 NocoBase 数据
```
...
发布脚本在本机构建 AMD64 镜像并推送腾讯云 CCR，同时生成配套部署包。包内保留后端源码用于追溯，但生产服务器只拉取镜像，不执行构建。包不含离线 Docker 镜像 tar、本机配置、参数库或历史记录。按用户要求，本次不做机器上的 Flow 导入验证；以下操作留到具备部署机器后执行。
...
将 ZIP 顶层目录的内容放到 `/userdata/holo/pyfiles/m4/`，使 backend、node_red 和 secrets 直接位于 m4 下，不要再嵌套一层发布包名。以下命令由部署人员在服务器上执行：
...
│   └── node-red.env          # Node-RED 环境配置（服务器创建）
├── backups/                  # 按需备份
└── 部署手册.md
```
...
发布流程参照 M3 的 `m3/deploy/build-and-push.sh`。默认仓库沿用 `ccr.ccs.tencentyun.com/taidai-holobase-168/omnipower_vifa`，M4 标签统一使用 `m4-` 前缀，避免覆盖 M3 的版本。环境变量名沿用 M3 的 `CRR_IMAGE`；腾讯云服务实际名称为 CCR。

### 本地发布

先将准备发布的代码提交，使 Git 工作区干净。在实际代码仓库 `vifa/` 中执行：

```bash
docker login ccr.ccs.tencentyun.com
VERSION=m4-0.1.0 bash m4/deploy/build-and-push.sh
```

Docker 使用已有登录或凭据助手；脚本不读取或保存登录密码。可通过 `CRR_IMAGE` 更换仓库，`VERSION` 指定新的 `m4-` 版本标签，`RELEASE_DIR` 指定尚不存在的输出目录，`PYTHON_IMAGE` 指定构建基础镜像。平台固定 `linux/amd64`，适配本地 Mac 构建、生产 AMD64 运行。

脚本导出指定 Git 提交的快照，按白名单生成部署包，在本地 `buildx build --load`，核对镜像架构后推送两种标签并检查远端 manifest：

- 发布标签：`omnipower_vifa:m4-0.1.0`。
- 提交标签：`omnipower_vifa:m4-<12位Git提交号>-amd64`。

默认输出到仓库旁 `outputs/m4-0.1.0-<12位提交号>/` 及同名 ZIP。包内 `release.json` 记录完整提交号和镜像，`backend/.env.example` 自动填入提交标签。生产建议使用该提交标签，便于固定版本和回滚。构建或推送失败时脚本退出非零，不打印发布成功；先排查错误，再使用新的 `RELEASE_DIR` 重试。生成了 ZIP 不代表镜像已经推送成功。
...
生产 Compose 不包含 build 配置。`platform` 固定 `linux/amd64`，默认端口绑定 `127.0.0.1:8844`，命名卷 `vifa-m4-data` 挂载到 `/data`。修改宿主端口 `M4_BIND_PORT` 后，宿主机运行的 Node-RED 也要更新 `M4_BACKEND_URL`。脚本只发布镜像和文件，不连接或启动生产服务器。
...
## 5. 配置 Node-RED 的访问 Token 和后端地址

参考 `node_red/env.example`，将以下四个变量加入**现有 Node-RED 进程/容器的环境**：
...
可用下面的服务器命令创建受保护的环境文件。执行前修改两个示例 Origin；脚本仅首次创建文件，不覆盖既有配置，也不打印 Token：

```bash
sudo install -d -m 700 /userdata/holo/pyfiles/m4/secrets
sudo python3 - <<'PY'
import os, secrets
fd = os.open('/userdata/holo/pyfiles/m4/secrets/node-red.env', os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
with os.fdopen(fd, 'w') as output:
    ...
    output.write('M4_BACKEND_URL=http://127.0.0.1:8844\n')
PY
```

- **Node-RED 作为宿主机服务运行：** 在其实际 systemd 服务配置的 `[Service]` 中添加 `EnvironmentFile=/userdata/holo/pyfiles/m4/secrets/node-red.env`，然后 `systemctl daemon-reload` 并重启该 Node-RED 服务。服务名沿用现有部署，不照抄不确定的名字。
- **Node-RED 已用 Docker Compose 运行：** 在现有 Node-RED 服务添加 `env_file: /userdata/holo/pyfiles/m4/secrets/node-red.env`，由有权限读取该文件的部署用户重建该服务，并保留原有卷、端口与配置。单纯 `docker restart` 不会注入新增环境变量。

如果 Node-RED 在 Docker 容器中，`127.0.0.1` 指向 Node-RED 自己。将环境文件里的后端地址改为 `http://m4-api:8844`，并让现有 Node-RED 加入后端的 Docker 网络。默认网络名为 `vifa-m4_default`，可在 Node-RED 原 Compose 中追加：

```yaml
services:
  node-red:                       # 替换为现有服务名；合并到原配置
    env_file: /userdata/holo/pyfiles/m4/secrets/node-red.env
    networks:
      - default                   # 保留已有业务网络
      - m4-network
networks:
...
先启动后端创建网络，再更新 Node-RED。若自行修改了后端 Compose project 名，网络名也随之调整。本手册按 Node-RED 与 M4 同服务器部署；不要为跨服务器连接直接将后端端口开放到公网。

Node-RED 缺少 Token/Origin 或配置格式不正确时，Flow 会拒绝请求并返回 503。访问 Token 要求 32–512 个字符，仅允许字母、数字和 `._~-`，建议使用上述 64 位十六进制值。

## 6. 将 HTML 和 API 路由放入 Node-RED

有部署机器后，在现有 Node-RED 中导入 `node_red/m4_customer_flow.json` 并 Deploy。它使用内置 HTTP In、Function、Template、HTTP Request、HTTP Response 和 Catch 节点，不要求额外安装节点包。

页面链路是 `GET /m4 → 校验页面 Token → Template → HTTP Response`；API 链路是 `GET/PUT/POST /m4-api/... → 校验 Bearer → 限定接口 → HTTP Request → HTTP Response`。完整 Flow 已嵌入同一份 HTML，不需要再额外粘贴一次。
...
docker run --rm --platform linux/amd64 --network none --read-only \
  ...
  -v vifa-m4-data:/data:ro ccr.ccs.tencentyun.com/taidai-holobase-168/omnipower_vifa:m4-0.1.0 \
...
- 更新 HTML：使用同一次发布包的 HTML 替换原 Template 内容并 Deploy；接口变化时同步更新配套 Flow。
- 更新后端：本地提交后用新的 `VERSION=m4-…` 运行发布脚本。服务器确认无运行中任务并备份，将新包内容合并到原安装目录，保留已有 `.env`、Token 和数据卷；仅将 `backend/.env` 中的 `M4_IMAGE` 改为新包 `release.json` 对应的提交标签，再执行以下命令。
- 回滚：停止新容器，将 `M4_IMAGE` 改回保留的旧提交标签，拉取并启动；数据格式变化时切回兼容的原卷/备份，不假定旧代码能读取新记录。
...
如果服务器无法访问 CCR，可从已经完成本地构建的镜像导出，再上传服务器。以下标签要与实际发布版本一致：

```bash
# 本地构建机：发布脚本 --load 留下该镜像。
docker save -o vifa-m4-amd64.tar ccr.ccs.tencentyun.com/taidai-holobase-168/omnipower_vifa:m4-0.1.0
# 服务器：
docker load -i vifa-m4-amd64.tar
# .env 的 M4_IMAGE 设置为已导入的同一完整标签后：
docker compose up -d --no-build m4-api
```

当前 ZIP 不包含镜像 tar。已执行的检查及尚未执行的真实构建/推送范围见 `验收记录.md`；现场部署状态以服务器实际结果为准。
...
输出目录必须不存在。生成脚本按白名单复制后端源码，生成 HTML、Flow、手册、校验清单及 ZIP，不复制现场凭据和运行数据。

正式发布以脚本从干净 Git 提交生成的配套镜像、`release.json`、HTML 和 Flow 为准。此前固定 `620ef58` 的 TCP/Socket 示例包不包含后续账单功能；不要混用旧包 HTML 与新接口。手动执行 `build_package.py` 只生成文件，不构建或推送镜像。


## 账单统计更新提示

账单统计需要同时发布当前 HTML、后端 `m4_settings` 包和包含 `bills` GET 白名单的 Node-RED Flow。更新后端后重启服务。查询为 `/m4-api/stations/{station_id}/bills?month=YYYY-MM`，上游 Token 需具有 `t_es_stat:list`、`t_es_count_stat:list` 只读权限。详细字段及日月差额口径见源码 `m4/账单统计说明.md`；该功能不写统计表、不触发求解或设备下发。旧发布包不会随源码编辑自动更新。
