...
Pushes the fixed 0.1.0 tag and m4-<git-sha12>-amd64 for rollback. Never starts production services.
EOF
}
...
python3 "$snapshot_dir/m4/deploy/build_package.py" \
  --output "$release_dir" --image "$version_ref" --revision "$commit_sha"
