...
- 本次提交范围为晚峰保留v2及回归、并行MAPE、只读预览、页面重构与EMS原计划展示、部署覆盖文件和相关说明。运行产物与本地凭据不提交。

## 2026-09-10 采用EMS时展示原始配置

- 用户明确要求采用EMS就展示EMS原计划。线上HTML在comparison.status=ems时读取同版本baseline.schedule（后台已换算站级功率），时间条、储能曲线、当前/下一动作及选中详情均用原配置；优化分支仍用recommended.plan。购电/SOC/费用/电量指标保留后台模拟结果，明确EMS配置功率和模拟购电说明，缓存升级v5。
- 电站1真实只读结果浏览器确认19:00–21:00原配置放电120kW，详情仍独立显示模拟购电482kW；原计划读缺失、跨站、版本错误不伪造。脚本语法、原配置展开、跨午夜/全天/重叠/身份拒绝检查通过。未改求解器、未下发、未重建镜像，线上需同步HTML。

## 2026-09-10 电站1晚间EMS显示口径核对

- 用户询问沿用EMS为何19:00–19:15放电。只读8847代理取得线上daily-plan及daily-inputs，推荐ems与baseline逐点一致；原schedule包含19:00–21:00，柜级60kW、双柜站级120kW。显示的是ems_rule_simulation，并非原始配置时段表。
- 保存计划及最新基线均19:00前SOC2.214559%，模拟120kW放电约6.0864分钟后于19:06:05到安全下限1%；15分钟平均48.6912kW，19:15后待机。没有混用优化结果；24小时策略时间条把模拟非零平均功率分组，导致用户误以为原配置只有15分钟。尚未改展示或控制规则。只读证据outputs/m4/evaluations/2026-09-10-station1-ems-display/。

## 2026-09-10 运行记录返回按钮移至左上

- 后续按用户要求去除页面“M4 优化调度控制台”标题，图表说明仅保留日期/00–24时段/15分钟粒度，移除北京时间、非实测及光伏缺口估算文案；数据和计算口径不变。
- 按用户要求将线上HTML的back-console从记录页底部移到记录内容左上、标题之前，保留原返回事件。8847实际渲染及返回交互确认；未重建镜像，此次页面修改需另行同步Node-RED模板。

## 2026-09-10 晚峰保留 v2 镜像已推送（待用户部署）

- 用户要求更新镜像并输出部署命令。新标签 m4-late-peak-reserve-v2-20260910-amd64 已推送原 CCR；远端回读摘要 sha256:ca2e007cb5a0f1cf83dde296ce5c125439eb70dff6a799ee5eddfb61db0d0b8d，平台 linux/amd64。
- 基于 parallel-inputs 镜像，用白名单流式构建更新57个源码/HTML文件；镜像内哈希逐一一致、API导入通过，断网运行晚峰保留及并发加载11项测试通过。保留前轮本机102项测试及现场快照试算证据；本轮不重复线上求解。
- 新覆盖文件 m4/deploy/m4-late-peak-reserve.override.yaml 固定新摘要及1panel-network的m4-api别名，保留原Compose/PV/M3 MAPE挂载层。发布源码清单位于 outputs/m4/releases/late-peak-reserve-v2-20260910/source-manifest.json。
- 未远程部署、未提交Git、未下发。用户部署后需重新生成电站2优化计划；Node-RED页面仍需独立同步 m4/web/M4优化调度控制台-线上版.html，单独更新Docker不会替换Node-RED模板。

## 2026-09-10 晚峰保留规则测试及现场快照试算通过

- 用户新授权“运行测试或试算”，覆盖此前不测试要求。67项现有优化器测试、7项新增晚峰保留回归、28项输入/候选/PV测试共102项通过，git diff --check通过。新增测试文件m4/tests/test_m4_late_peak_reserve.py；合成复现将测试MIP gap设0确保能分辨小额回补收益，不修改生产容差。最初测试夹具/断言失败已修正并重跑通过。
- 只读取得电站2已保存run35a2c099-b58f-4d39-a2f0-7649dbc9bd68，同输入分别本地重放v1/v2。v1重现19:00充电44.6818kW（11.1705kWh），晚峰最低1.65048%；v2三候选optimal，19:00待机且19:00后充电0，晚峰起点至日末至少2%，无需量超限，独立复核全通过。
- 新cost候选2862.0099446元，旧2857.5637303元（约+4.45），EMS3305.1698041元，预计节省443.1598595元。旧快照由v2校验在75点拒绝。v1本机重放status feasible，未冒充optimal。未改现场参数、未线上求解、未下发、未写正式历史。
- 证据outputs/m4/evaluations/2026-09-10-late-peak-reserve/，验证说明.md及输入/结果/比较/摘要完整保留。新代码尚未构建推送部署。

## 2026-09-10 晚峰保留日末电量已实现（未运行/部署）

- 用户明确要求“晚峰放电时就保留日末所需电量”。新增peak-reserve-v2：按实际feng标签识别最后一个连续峰段，从该段起点状态到日末，所有电量状态硬约束不低于capacity×terminal_soc_min_pct/100。原日末下限max(EMS末SOC,推荐下限)不变；不单独禁止平段充电，不修改安全参数或EMS基线。
- model新增具名late_peak_terminal_reserve约束；validation独立正向定位最后峰段并校验起点和后续能量。contracts保留默认v1以维持历史序列化/语义，新daily_baseline显式v2；daily_policy/selector升v2、模型版本跟随策略。新服务会使旧当前计划失效，历史v1仍按旧规则。
- 主线上HTML同时识别v1/v2且核对请求版本；旧后端结果明确提示晚峰保留未生效，缓存命名空间v4。后端源码未构建/推送/部署，8847仍代理旧线上数据，需要新镜像部署后重新生成计划。
- 按用户持续“不测试”要求，仅源码审查，没有运行测试、语法检查、试算或下发。不声称新计划已验证。实施计划追加到现有docs/m4/plans/2026-09-09-m4-peak-reserve.md，配置说明已更新。

## 2026-09-10 19:00短充电来源核对

- 用户质疑时间条19:00–19:15充电是否求解器问题。只读发现station2最新计划已于18:40:48北京时间更新（先前“仍旧光伏来源”结论不再适用），PV来源与当前daily-inputs一致。
- 该计划确有19:00充电44.681820784965836kW，18:45时段末SOC1.6504774555%，19:00时段末恢复2%；安全下限1%、偏好下限2%、peak_reserve终端下限2%，19:00为平价0.65856875。是实际优化计划为满足日末2%回补，不是图形生成假动作；尚未修改规则或求解。
- station1同时间则为EMS基线放电48.6912kW至安全下限1%，需区分站点。未运行测试、未重新计算或下发。

## 2026-09-10 改为原始m4-static-dashboard结构（去AI）

- 用户指定Downloads/m4-static-dashboard.html，沿用现有主题，其余按参考结构。线上HTML重排为顶部工具/站点切换、调度总览、预测与调度、24小时策略、储能柜调度计划、策略执行日志；AI建议/置信度不导入。最新参考优先恢复柜表SOC，但图中无SOC。
- 核心daily输入/方案/校验/调度计算保持原实现，仍是当天00–24小时而非文案冒称未来24h；模拟按钮仍为生成优化计划，未冒称自动执行。原记录查询通过全部日志打开、返回控制台；参数按钮打开既有约束。
- 柜级状态新增独立GET inputs，预览白名单放行并重启8847；SOC及状态可读，目标功率/目标SOC未分配、当前柜功率未提供，明确占位不等分站级功率。读取不阻塞主图。
- 最近日志复用当前daily-plan和历史GET；发现线上历史接口部分unreadable，显示最新可读取计划及异常提示，不展示虚构执行结果。实际已查看桌面渲染，未运行测试、未部署。线上需同步HTML；本地预览仍只读，不接受POST。

## 2026-09-10 当前策略与查看时段解耦

- 用户反馈当前策略不随时间变化：原Hero使用selected（默认18:00）且没有时钟刷新。现当天Hero按北京时间当前15分钟点读取，整刻自动切换、恢复前台立即刷新；滑块/图点击仅改变下方详情。非当天页面明确标为所选日期/时段，避免冒充当前策略。
- 当前策略仍是已加载计划对应时段，自动切换不触发预测、求解或设备指令。未运行测试。

## 2026-09-10 M4再次隐藏SOC并核对线上数据

- 用户明确不需要SOC，移除主图曲线/图例、顶部时段SOC与滑块详情SOC，保留计算和历史记录。功率图恢复完整高度，顶部剩四项。
- 线上只读核对两站daily-inputs均200、can_compare=true、missing=[]，约2.4s/3.6s；并非接口失效。station1保存计划为ems（未找到更低费用且满足约束方案）；station2保存优化仍用旧pv-reference版本，当前页面会按来源保护回退最新EMS基线，需线上重新生成匹配当前光伏来源的计划。未代为求解、未运行测试。

## 2026-09-10 M4按用户文字结构图收拢

- 最新结构优先：单列顶部策略卡（五项计划数据及下一动作）、电价/储能动作时间条、SOC与购电储能/需量图、点击时段详情、四项今日结果。右侧卡片取消展示，测算口径和旧三张策略卡隐藏/移除。按用户新结构恢复SOC曲线并绘制已有需量目标，不更改策略或API。
- 所有动作及数值仍为计划，未虚构光伏充电来源或原因。滑块、时间条和点击图均选择96个实际计划时段。未运行测试，未部署。

## 2026-09-10 M4按用户参考HTML重构展示

- 依据Downloads/m4_mock_redesign.html调整线上HTML：所选时段动作Hero、左图右侧摘要/输入快照状态/后续4小时、可点击动作时间条、底部原费用电量指标。新展示均来自已有plan，未引入Mock数值，未改接口/调度策略。SOC曲线继续隐藏，功率共用正负轴。
- 本地8847已切换线上只读API代理，--online使用服务端既有凭据，禁止POST，凭据不进HTML或URL。按用户要求已向8844/8846/8848/8850/8874旧服务发送停止指令；仅保留8847。用户要求不运行测试。
- 当前HTML后续多次修改均晚于已发布镜像；生产需单独同步Node-RED模板。独立Demo未被采用。

## 2026-09-10 M4左侧边距对齐

- 用户确认与M1–M3对齐。按M3最终main.app规则，线上HTML外层改为靠左、最大1480px，桌面28px留白，720px及以下15px；解决1920px宽屏居中导致238px左侧留白。
- 浏览器12组尺寸/主题布局断言通过，已查看桌面/手机实际渲染，无页面横向溢出；未修改数据逻辑。此改动晚于已推送镜像，生产需同步Node-RED HTML模板。

## 2026-09-10 M4并行加载镜像已推送

- 用户要求构建推送并输出部署命令。已推送CCR标签m4-parallel-inputs-20260910-amd64，远端摘要sha256:d0725f081da3a991c9b266ad06a0cba0cd5603759364afd64ff88125765dfab5，回读确认linux/amd64。
- 继承已验证pv-daily-readonly基底，白名单57个源码/HTML文件哈希一致；新镜像内51项断网测试通过，API导入通过。包含并行MAPE、PV来源一致性及固定详情关闭按钮，未含凭据/运行数据。
- 新增m4/deploy/m4-parallel-inputs.override.yaml，最后覆盖镜像并持久化1panel-network别名m4-api。未远程部署；Node-RED HTML仍需独立同步。此次未提交Git。

## 2026-09-10 M4负荷预测与MAPE并行读取

- 用户确认优化电站1加载。forecast_source先校验入参，再以一个后台线程读取MAPE，主线程读取滚动/手动负荷；组装结果前等待gate完成，沿用同站同now、点数、门槛、版本摘要和异常规则。失败时线程收尾，不留下悬空请求。
- 新增4项并发握手/入参拒绝/门槛失败/异常收尾测试，相关46项通过。真实只读计时：两路约0.017s同时启动，负荷取数约1.18s，MAPE约3.02s，总约3.04s，96点完整且gate ready。此为本机公网测量，不代表生产固定时延。
- 未构建/推送/部署此次并发优化，线上仍需新M4镜像。未触发预测、求解或设备指令。

## 2026-09-10 调度详情关闭按钮固定

- 用户反馈长记录需回滚顶部才能关闭。线上HTML将详情弹窗改为固定标题/关闭按钮+独立滚动内容区，保留关闭和键盘行为。
- 一次性浏览器验证：96行滚至底部按钮位置不变且可关闭，1440/834/390/320px、明暗主题8组通过。仅修改页面，需同步Node-RED现有HTML模板，未修改或部署后端。

## 2026-09-10 电站1安全SOC调整

- 用户明确授权将电站1安全范围改为1%–98%。通过认证settings接口以expected_revision=4保存，回读revision=5，下限1、上限98，其他参数全部不变。
- 只读daily-inputs复核：零点SOC1.95%，can_compare=true、missing=[]，EMS基线ready（5241.05元）。未修改实测SOC、未生成新优化方案、未下发设备。

## 2026-09-10 M3→M4线上联调及旧方案来源修复

- 线上M3 latest与M4 daily-inputs/plan返回200；M3与M4批次相同，56点逐点一致，40点补零，M4 can_compare=true、missing=[]，负荷MAPE7.1821%。全程GET，没有触发新预测/求解。
- 发现线上daily-plan仍基于旧pv-reference；独立浏览器测试先复现旧优化被当作当前方案展示，再修复HTML：本站当前PV来源版本不一致时回退新输入EMS基线，历史记录保留；缓存v3。
- 本轮未修改已部署后端/M3算法。线上页面版本仍未验证，/m4认证头401，URL凭据检查被自动审批拒绝且未执行；新HTML需同步Node-RED现有模板。明早06:00定时结果尚未发生/验收，不声称定时闭环通过。

## 2026-09-10 M4部署502已由共享网络修复

- 用户回传新版容器healthy、健康接口200，但无页面API请求；用户执行`docker network connect --alias m4-api 1panel-network vifa-m4-m4-api-1`后确认恢复。故障位于Node-RED到M4的共享网络连接，而非容器启动或光伏算法。
- 仓库m4-pv.override.yaml显式配置nodered_shared网络别名m4-api；服务器需同步此网络配置或写入最后生效的覆盖文件，手动network connect不会持久到容器重建。未代用户重启或远程操作。

## 2026-09-10 M4光伏改为日级有效期

- 用户确认实施：按北京时间仅接受今天/昨天生成的完整已完成批次，替代固定2小时过期；不变更最新批次选择和未来生成拒绝。全天窗口重叠部分使用预测，缺口继续按零估算；整天无重叠/批次损坏/前天及更早批次拒绝，滚动候选仍需完整覆盖。
- 新策略m4-pv-daily-validity-v2进入来源版本，不再输出max_age_seconds=7200。新增6项时区/六点前后/昨日回退/过旧/未来/覆盖测试，相关53项通过。另有前轮已记录的滚动负荷MAPE夹具问题未涉及。
- 本机真实只读查询成功，run a7995e69-254f-48cd-aae4-8497e5af6cf5，9月10日09:46生成，10:00起滚动24小时；当天56预测点+40补零点。无生成请求/历史读取/设备下发。
- 已构建vifa-m4:pv-daily-readonly-20260910，沿用M4当前MAPE基础镜像，启动入口改为迁移后的m4.settings.api:create_app；代码白名单与新HTML入镜像，未带凭据。镜像sha256:9120dc59bbd8e167e8ea32bbf57f6e677a58cd0f08fd209186961f99051fa155，linux/amd64，镜像内53项断网测试通过。未推送/部署。

## 2026-09-10 用户确认光伏窗口缺口按零估算

- 用户明确要求M4光伏缺口补0。全天输入对已验证完整的M3批次按自然日对齐，预测窗口以外的时段补0 kW；保存gap_policy、forecast_coverage_points、zero_filled_points/at并绑定版本，计划source_versions与页面显示补零点数。
- 整批缺失、批内缺点/损坏、读取失败、整天无重叠不补零；2小时新鲜度门槛仍保留。仅全天回算采用该假设，滚动候选覆盖规则不变。M4不主动生成光伏。
- 定向47项测试通过，另扩大测试中的1项滚动负荷MAPE夹具失败仍保留。未构建/部署新版M4，常驻本机API未重启。

## 2026-09-10 M4只查询M3光伏结果

- 用户要求接入M3预测结果并去除M4主动预测。已移除api/candidates中的PreparedInputs/PVService链路与生成客户端，pv_on_demand仅保留M3共用Socket传输；删除旧主动预测测试，新增只读来源与窗口测试。
- 全天输入不再调用7天光伏历史，改用已发布批次的只读接口并校验完整自然日覆盖。滚动入口继续读取M3发布表；缺失/过期/不完整提示等待M3，不补零/拼接/回退历史。电站1无光伏规则保留。
- 本机只读真实查询耗时0.27秒，返回2小时过期提示，仅查询energy_pv_forecast_runs，无POST或历史读取。06:00滚动批次不覆盖当天00:00–24:00，2小时时效未改；仍需后续明确M4计划窗口和日级时效。
- 浏览器缓存升v2；部署配置移除PV控制Socket，但镜像尚未构建发布，生产/本机常驻API均未重启。相关测试通过；扩大到滚动负荷测试有1项因当前MAPE测试数据缺失失败，未修改该负荷规则。

## 2026-09-10 M3 每日预测镜像已推送CCR

- 用户明确确认上传M3源码及训练快照到现有CCR仓库后，推送成功。标签m3-pv-daily-0600-20260910-amd64，远端摘要sha256:7dfe3c5387201efd3cefca3aa06ae39c1862c9e4ae15574e5429b5fb77ac122d，imagetools回读确认linux/amd64。
- compose.registry.yaml已固定新摘要；README部署命令用覆盖文件仅更新vifa-pv镜像和PV_DAILY_SCHEDULE_ENABLED=1，保留现有挂载。未部署生产、未触发真实预测、未修改M4。

## 2026-09-10 M3 每日预测镜像已本机构建

- 用户要求先构建镜像，M4稍后处理。已构建 `vifa-m3-pv:daily-0600-20260910`，linux/amd64，镜像ID `sha256:7dfe3c5387201efd3cefca3aa06ae39c1862c9e4ae15574e5429b5fb77ac122d`。基础为本机business-api-20260910版本，使用源码白名单流式构建与已校验训练快照；未复制凭据或完整源码到临时目录。
- 新镜像断网只读容器24项测试通过，训练快照SHA-256复核通过；本地Compose镜像标签同步，配置校验通过。构建清单位于outputs/m3/releases/pv-daily-0600-20260910/，不提交运行产物。
- 未推送镜像、未部署、未运行真实预测，M4代码不变；registry Compose仍旧摘要。每日定时需新版PV服务及PV_DAILY_SCHEDULE_ENABLED=1。

## 2026-09-10 M3 光伏每日06:00自动预测

- 用户要求M3每天早上六点预测。ManualJobs新增北京时间06:00分钟内的一次forecast提交，复用更新天气/预测/入库流程；PV服务可用PV_DAILY_SCHEDULE_ENABLED启停，Compose开启。原手动入口保留。
- 日级提交意图先持久化，服务锁+线程锁与手动任务互斥，失败/中断/不确定提交不自动重发；错过06:00分钟不补跑。线程随服务关闭；训练、模型、预测窗口与M4时效规则未更改。
- 新日调度测试覆盖时区、跨日、重启去重、忙碌、失败、提交不确定、线程关闭与配置开关。未部署/发起真实预测/设备下发；registry Compose仍旧镜像摘要，须发布新镜像后更新才能生效。每日06:00批次不能直接满足M4全天自然日覆盖及2小时新鲜度规则。

## 2026-09-10 恢复光伏展示并准备提交

- 用户明确撤回隐藏光伏的改动，恢复曲线、图例、消纳指标、供能策略卡及时段数值；后端光伏逻辑一直未停用。
- 用户要求先提交当前代码，再在M3提前生成光伏预测；仅授权预测任务，不涉及设备下发。页面回归（16组布局）、缓存蒙版、5项峰价指标及19项网关/MAPE测试通过。生成入口为服务端Node-RED的“2 · 手动生成一次预测（自动取新天气）”，公开路由仅GET查询；本机无光伏Socket，尚未发起预测。

## 2026-09-10 暂时隐藏光伏预测展示

- 用户尚未确定光伏方案，要求暂时隐藏。线上 M4 隐藏光伏曲线、图例、时段光伏数值、消纳指标及供能策略卡；保留预测负荷、购电和储能展示，两张策略卡自适应布局。
- 仅调整展示层，后端光伏历史读取及调度计算口径保留，不将光伏填零，不实施跨批次拼接。因此该隐藏操作不降低接口取数耗时。静态示例未修改。

## 2026-09-10 检查光伏预测替代历史查询

- 用户确认先检查是否已有可复用的当天光伏预测。只读查询 ES02 已完成批次及实际预测点：共4个 operational 批次，每批96点，但均为滚动24小时，没有单批覆盖9月10日自然日。
- 北京时间：批次4覆盖当天10:00至次日10:00，当天56点；批次3覆盖当天00:45至次日00:45，当天93点，缺00:00/00:15/00:30；批次2覆盖前日23:00至当天23:00，当天92点，缺最后4点；批次1当天91点。两个批次可以覆盖全天，但需新增明确的跨批次选择及来源追溯规则，不能直接替换既有单批读取器。
- 本轮未修改光伏数据源，未补零、未拼接批次、未触发预测任务。现有 load_pv_forecast 面向滚动窗口，有2小时有效期，不能直接套用于全天回算；历史查询仍保留。

## 2026-09-10 M4 首屏加载蒙版

- 线上页原生 HTML 内置加载蒙版，首次取数/无有效快照时显示，5 秒后提示读取较慢；缓存恢复后立即撤下，后台校验继续。成功、阻断、错误、非当天日期均收尾，晚到响应不撤下新请求蒙版；摘要初始化失败降级内存缓存。支持明暗主题与减少动画偏好。
- 本机并行只读计时：HTML 22ms、daily-plan 41ms、settings 42ms、control-sources 1.948s、daily-inputs 31.233s（单次测量）。主要耗时在当天输入读取，前端已并发请求；本轮未更改后端缓存/业务规则，不声称接口已提速。
- 缓存 E2E 增加蒙版显示、慢请求提示、缓存不遮挡及失败撤下检查。

## 2026-09-10 刷新与切站保留快照

- 用户反馈刷新、切站反复等待加载。线上页改为按电站+当天日期保留最近5分钟视图：内存复用，当前标签页 sessionStorage 支持刷新恢复；保存上次所选电站。首次未读过的电站仍需取数。
- 切站先恢复该站自己的图表/阻断状态，标记“缓存快照 · 正在更新”，后台仍读取并校验真实接口。核对期间 context 清空、生成按钮禁用；失败/过期/跨日不使用快照继续生成，失败时清除对应缓存。
- 同站同日进行中请求复用，快速切换不重复发起；保留 generation 隔离晚到响应。平台凭据仅通过 SHA-256 摘要分隔缓存，不保存原始 Token；无凭据的持久快照仅用于本机地址，其他不支持摘要的环境仅保留内存。
- 新 m4_station_cache_e2e.js 验证刷新恢复、切站即显、请求去重、晚到响应、生成禁用、凭据不落缓存及到期/错误清除；原 m4_reference_live_e2e.js 回归通过。真实本机预览只读检查，不触发求解或设备下发。

## 2026-09-10 削峰指标改为峰价时段最大购电功率

- 用户确认将首页“预计降低最大购电功率”改为“峰价时段最大购电功率降低”，排除谷/平时段，以峰与尖峰时段内的基准最大值减计划最大值；比例分母为同范围基准最大值，不使用单时段最大降幅。
- 线上页、静态示例、调度记录列及测算口径说明已同步。线上用匹配的原始 tariff_period（feng/peak/sharp）筛选；96 点不完整、时间不匹配、未知电价时段或无峰段时不回退全天统计，隐藏首页指标，记录显示缺失。保留负值（增加）的正确文案和零分母处理。
- 新 m4/tests/m4_peak_tariff_test.js 5 项通过，覆盖夜间全日峰值排除、两个不同时间最大值比较、尖峰、缺失时段、身份对齐及负降幅。不修改优化器、实际计划或设备指令。

## 2026-09-10 新版 M4 已接通 M3 当前任务，预览切换完成

- 用户确认修复。新增显式 `M4_M3_TRANSPORT=platform_gateway` 只读方式，M3CurrentResult 将两个固定 Worker 读取路径映射到生产 OPDash 已有用户认证网关，解封装 status/data；复用本机现有平台凭据，无凭据输出，无重定向。默认 Socket 模式和 MAPE 算法/30%阈值不变。
- 先写失败测试，再完成适配；`test_m4_m3_gateway` + `test_m4_load_accuracy` 共 19 项通过，涵盖原 Socket 读取、当前点数及求解门槛。真实 GET 核对两站同一 M3 当前任务成功。
- 电站 1 当前 run 3414fb4a-bf5a-4566-b94f-2b064a6488b5：前 59 点为 11.2852277%（与用户11.29%一致）；回填至60点后11.1339005%。电站2 run f98ae7a6-c749-4ddf-b1c0-e90488136a20：60点7.8578004%。均 ready，临时结果正常允许评估。
- 当前仓库新版 API 由本轮启动于 127.0.0.1:8848，PID 21305，启用 platform_gateway；旧目录 8846/PID1003 未停止。8847 预览已重启并改上游为8848，脚本默认也改为8848；地址仍 http://127.0.0.1:8847/m4。
- 新版真实 daily-inputs：station-2 can_compare=true；station-1 因零点SOC超出当前安全范围 can_compare=false，MAPE 已 ready。没有绕过此独立约束，没有生成真实求解或下发设备。部署与启动方式合并到 docs/m4/负荷MAPE门槛说明.md。

## 2026-09-10 MAPE 差异定位：本机预览误接旧服务

- 用户指出 M3 当前负荷 MAPE 为 11.29%，59/96 实际点、临时结果。该值低于 30%，临时结果本身不应被当前任务门槛拒绝（仍需同站同任务核对）。
- 实际只读证据：8846 API 返回 `policy=m4-load-mape-v1`、MAPE unavailable；PID 1003 于 10:45 启动，cwd 为旧 `/Users/hua/Documents/LVK/code/vifa/vifa`，命令为 `uvicorn m4_settings.api:create_app`。本轮 8847 预览代理沿用了这一旧进程，因此不能把它的旧评估缺失解释为 M3 当前任务没有 MAPE。
- 当前仓库实现为 `m4-current-task-load-mape-v2`，通过 M3 当前任务结果接口读取实时实际值 overlay。新旧口径不一致是已确认根因；阈值和新代码没有修改。
- 本机未发现 M3 Worker Socket（新 runtime/m3 无 sock，旧 runtime/m3 与 /run/vifa-m3 不存在）；部署文档指向生产 OPDash 网关，Worker Socket/专用管理 Token 在服务端。尚未将预览改接新版且配置有效 M3 通道的服务，未重启 PID 1003、未远程部署或下发。需修通连接后再验证同一任务的 MAPE，不能声称已修复。

## 2026-09-10 截图布局接入真实 M4 API

- 用户再次明确“下一步”接真实数据，严格保留 `m4-decision-1440-light.png` 的两 Tab 布局。`m4/web/M4优化调度控制台-线上版.html` 改为截图版真实接口入口，静态 HTML 保留作示例。未恢复账单、候选方案或设备状态等额外区块。
- 读取 settings/control-sources/daily-inputs/daily-plan；仅用户点击时 POST daily-plan，并回读状态。按站点、日期、配置/控制版本及全天 96 点校验，缺数或跨日隐藏图表和收益；允许用有效的当天 EMS 基线回退过期优化计划，不补模拟数据。费用使用真实 EMS 净电费基准，净节省计入两方案吞吐费用差。
- 记录读取 decision-history/decision-results 并合并最新 daily-plan，保留筛选、完整时段详情。现有接口未暴露更早的 daily-plan 归档，因此不声称包括全部全天任务档案；单站最多读取 2000 条历史，超限有提示。
- 本机 8846 是旧页面服务，新版预览通过 `python3 m4/scripts/preview_console.py --port 8847 --upstream-port 8846` 复用其 API，地址 http://127.0.0.1:8847/m4，仅监听本机；不重启旧服务。预览只允许已列出的查询接口与同源手动 daily-plan 计算。
- 实际只读联调成功：两站均提示“负荷MAPE无法有效评估，未允许求解”，按钮禁用，图表不伪造；读取 13 条记录，打开实际记录 96 时段详情，无浏览器错误。未触发真实计算、生产部署或设备下发。
- 新 `m4/tests/m4_reference_live_e2e.js` 验证 API 契约、费用、图表/滑块、96 点详情、单 POST、同源凭据、缺数/过期/配置/切站隔离和 16 组明暗响应式渲染。旧五 Tab 页面专用 E2E 不再作为新版验收，未执行全量后端测试；后端业务规则未修改。未 Git 提交。

## 2026-09-10 M4 两 Tab 静态展示版

- 用户最新明确仅展示 `m4-decision-1440-light.png` 截图中的内容；撤回后续扩展的线上前端整合与配套测试修改，线上版恢复本轮前状态。保留静态两 Tab，不追加账单、候选方案或设备状态入口。
- 用户确认先做静态展示，重构 `m4/web/M4优化调度控制台.html`：Tab1 电价策略决策，Tab2 调度记录。参考 Downloads 中的静态 HTML，不把其 AI 置信度或执行状态当作现行业务契约。
- 展示电价、光伏、购电、储能充放电及 SOC 联动；净节省与削峰从同组示例数据计算。记录支持筛选、96 时段详情、模拟完成/阻断；按用户要求不提供本地导出。
- “充”暂指储能充电，独立充电桩未建模。全部为模拟建议、未下发设备，新增记录仅在页面会话保存；真实线上版、优化器、接口未修改。
- 本地浏览器验证通过：两 Tab、键盘切换、电站、时间滑块、生成记录、筛选/空态/详情及 1440/834/390/320px 明暗主题；两站功率平衡、SOC 边界、日终电量检查通过。未运行后端测试；未提交、部署。
- 相关设计说明已合并到 `docs/m4/M4优化调度控制台概要设计.md`。之前目录迁移及 M2 修复已提交 `d87f428`，VIFA 规范与 M1 调整为 `ee79b06`，均未推送。
