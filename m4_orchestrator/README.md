...
- 数学决策：`m4_selection.solver_selection` 位于 `optimization_result` 与最终选择之间，只能选择已有且有效的 `profile_id`，不能修改 96 点计划。
- EMS：适配器位于候选明确选择且完成确定性复验之后；只有有效候选才能转换为下发 DTO。

数学决策及本地 FastAPI 适配已在独立包中实现，见[求解器决策说明](../m4/求解器决策说明.md)。本包不包含这些服务或 EMS 适配器。
