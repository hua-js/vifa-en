...
from m3.worker.domain import custom_forecasting
...
def make_dataset(
    ...
    return CustomTrainingDataset(
        ...
        source_available_start=times[0].to_pydatetime(),
        source_available_points=len(times),
...
class CustomForecastAnchoringTests(unittest.TestCase):
    ...
    @patch("m3.worker.domain.custom_forecasting._forecast_frame")
...
class LoadDispatchSelectionTests(unittest.TestCase):
    ...
    @patch("m3.worker.domain.custom_forecasting.MSTL")
    ...
    def test_fifteen_minute_automatic_models_participate_on_latest_week_holdout(
        ...
        with self.assertLogs(
            "m3.worker.domain.custom_forecasting", level="INFO"
        ) as captured_logs:
...
class LoadForecastingTests(unittest.TestCase):
    ...
    @patch("m3.worker.domain.custom_forecasting.weekly_profile_values")
    ...
    @patch("m3.worker.domain.custom_forecasting._forecast_load_model")
