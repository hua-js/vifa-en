...
from m3.worker.clients import source_api
...
class SourceClientTests(unittest.TestCase):
    ...
    def test_retries_429_with_a_fresh_request_and_retry_after_delay(self):
        ...
        with (
            patch("m3.worker.clients.http.random.uniform", return_value=0.0),
            patch("m3.worker.clients.http.time.sleep") as sleep,
        ):
    ...
    def test_ignores_malformed_or_nonfinite_retry_after_values_safely(self):
        ...
        for retry_after in ("not-a-delay", "NaN", "+inf", "-inf"):
            with self.subTest(retry_after=retry_after):
                ...
                with patch("m3.worker.clients.http.random.uniform", return_value=0.0), patch(
                    "m3.worker.clients.http.time.sleep", side_effect=record_finite_sleep
                ):
                    points = make_client(
