...
PRODUCTION_ENV_README = ROOT / "docs/m3/deploy/production-env/README.md"
GUIDE = ROOT / "docs/m3/部署说明.md"
RUNTIME_LOCK = DEPLOY / "requirements.lock.txt"
DOCKERFILE = ROOT / "m3/deploy/Dockerfile"
DOCKERIGNORE = ROOT / ".dockerignore"
COMPOSE = ROOT / "m3/deploy/compose.yaml"
ENTRYPOINT = DEPLOY / "container-entrypoint.sh"
PRODUCTION_FLOW = ROOT / "m3" / "node_red" / "m3_production_gateway_flow.json"
RUNBOOK = ROOT / "docs/m3/AMD64三域Docker部署手册.md"
...
class DeploymentArtifactTests(unittest.TestCase):
    ...
    def test_image_uses_python_312_hash_lock_and_non_root_runtime(self):
        ...
        self.assertIn("m3/deploy/requirements.lock.txt", dockerfile)
        self.assertIn(
        ...
        for forbidden in (".local/密钥.txt", ".env", "raw-source.token", "Bearer "):
    ...
    def test_compose_renders_two_hardened_services_without_tcp_or_docker_socket(self):
        ...
        self.assertEqual(Path(worker["build"]["context"]), ROOT)
        self.assertTrue((ROOT / worker["build"]["dockerfile"]).is_file())
        run_mount = next(volume for volume in worker["volumes"] if volume["target"] == "/run/vifa-m3")
        self.assertEqual(Path(run_mount["source"]), ROOT / "run")
    ...
    def test_entrypoint_owns_only_its_selected_socket_and_one_uvicorn_worker(self):
        ...
        self.assertIn("m3.worker.main:app", script)
        self.assertIn("m3.worker.persisted_dashboard_app:app", script)
    ...
    def test_node_red_dashboard_exec_calls_only_the_fixed_host_dashboard_socket(self):
        ...
        for forbidden in ("python", "docker", "m3/forecast_api.py"):
