...
from m3.worker.domain.custom_forecasting import CustomChampion
from m3.worker.domain.custom_load_profiles import (
    LoadCandidateScore,
    weekly_profile_values as real_weekly_profile_values,
)
from m3.worker.domain.custom_training_data import CustomWeekSummary
...
class CustomForecastServiceTests(unittest.TestCase):
    ...
    def test_one_minute_run_models_soc_on_five_minutes_then_interpolates_output(self):
        ...
        with (
            patch(
                "m3.worker.services.custom_forecast_service.build_custom_training_dataset",
                side_effect=lambda _points, unique_id, config: stub_dataset(
                    unique_id, config
                ),
            ),
            patch(
                "m3.worker.services.custom_forecast_service.select_custom_champion",
                side_effect=stub_champion,
            ),
            patch(
                "m3.worker.services.custom_forecast_service.forecast_custom_series",
    ...
    def test_execute_persists_weekly_load_evidence_and_weekly_baseline(self):
        ...
        with self.assertLogs("m3.worker.custom_forecast", level="INFO") as logs:
    ...
    def test_execute_persists_selected_and_realized_load_fallback_evidence(self):
        ...
        with patch(
            "m3.worker.domain.custom_forecasting.weekly_profile_values",
