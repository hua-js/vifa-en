...
交付压缩包包含 Flow、服务源码、Dockerfile、独立 Compose、训练快照及逐文件 SHA-256。按用户最新确认，以现有 AMD64 镜像 `vifa-m3:0.1.0` 为基础合并为 `vifa-m3-pv:2026-09-09`，同一镜像支持三个运行模式：`worker`、`dashboard`、`pv`。前两种转交原 M3 入口，光伏模式启动私有 PV Socket 服务；默认模式为 `pv`。
...
| 文件/目录 | 用途 |
|---|---|
| `/userdata/holo/pyfiles/vifa-pv/app` | 解压后的部署包根目录（含 `compose.yaml`） |
...
4. 在部署包根目录执行：
   ...
   docker compose build vifa-pv
   docker compose up -d vifa-pv
   docker compose ps
   ```
